...
void Diagram::funcDecl() {
    ...
    if (t != RPAREN) {
        while (true) {
            ...
            dataDetails paramDetails = { false, paramType, 0 };
...
void Diagram::constDecl() {
    ...
    if (!Tree::isAssignCompatible({ false, constType, 0 }, exprDetails)) {
    ...
    dataDetails constDetails = { false, constType, 0 };
    try {
...
void Diagram::varDecl() {
    ...
    while (true) {
        ...
        dataDetails varDetails = { false, varType, 0 };

        // Проверка на массивы
        t = peekToken();
        if (t == LBRACKET) {
            nextToken(); // '['
            varDetails.isArr = true;

            dataDetails sizeDetails = expr(); // размер массива

            // Проверка, что размер массива - целочисленный
            if (sizeDetails.type != t_int && sizeDetails.type != t_short && sizeDetails.type != t_long) {
                throw SemanticError("Семантическая ошибка: размер массива должен быть целочисленным");
            }
        ...
        // Добавляем переменную/массив в дерево
        try {
            nodeType nodeType = varDetails.isArr ? Arr : Var;
            tree->addSymbol(nodeType, varName, varDetails, false, sc->getLineCol().first, sc->getLineCol().second);
        }
        catch (const std::runtime_error& e) {
            throw SemanticError(std::string("Семантическая ошибка: ") + e.what());
        }
...
void Diagram::stmt() {
    ...
    if (t == IDENT) {
        ...
        if (t == LBRACKET) {
            // Индексация массива
            if (identNode && !identNode->details.isArr) {
                throw SemanticError("Семантическая ошибка: '" + identName + "' не является массивом");
            }
            nextToken(); // LBRACKET
            dataDetails indexDetails = expr(); // индекс

            // Проверка типа индекса
            if (indexDetails.type != t_int && indexDetails.type != t_short &&
                indexDetails.type != t_long) {
                throw SemanticError("Семантическая ошибка: индекс массива должен быть целочисленным");
            }
        ...
        if (t == ASSIGN) {
            ...
            if (identNode) {
                ...
                if (isArrayAccess) {
                    lhsDetails.isArr = false; // При доступе к элементу массива работаем с базовым типом
                }
...
void Diagram::returnStmt() {
    ...
    if (t != SEMI) {
        ...
        if (!Tree::isAssignCompatible({ false, currentReturnType, 0 }, retDetails)) {
...
dataDetails Diagram::add() {
    ...
    while (t == PLUS || t == MINUS) {
        ...
        // Для арифметики используем больший тип
        left.isArr = false;
        left.size = 0;
...
dataDetails Diagram::prim() {
    ...
    if (t == IDENT) {
        ...
        if (t == LPAREN) {
            ...
            result.isArr = false; // Результат функции не является массивом
            return result;
        }

        // Индексация массива
        if (t == LBRACKET) {
            if (!identNode->details.isArr) {
                throw SemanticError("Семантическая ошибка: '" + identName + "' не является массивом");
            }
            nextToken(); // '['
            dataDetails indexDetails = expr(); // индекс
            ...
            // Возвращаем базовый тип массива
            dataDetails result = identNode->details;
            result.isArr = false; // Элемент массива не является массивом
            return result;
        }
    ...
    return { false, t_int, 0 }; // Заглушка для компилятора
}