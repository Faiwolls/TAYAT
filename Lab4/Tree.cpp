...
Node::Node(nodeType k)
    ...
    name(""), details({ t_int }), isConst(false), line(0), pos(0)
{
}
    ...
    int line, int pos)
{
    ...
    // Проверка на дублирование с именами функций
    child = getRoot()->right;
    while (child) {
        if (child->type != Scope && child->name == name) {
            throw runtime_error("Повторяющийся идентификатор функции '" + name + "'");
        }
        child = child->left;
    }
...
void Tree::printRec(const Node* node, int indent) const {
    ...
    if (node->type == Var || node->type == Const || node->type == Func) {
